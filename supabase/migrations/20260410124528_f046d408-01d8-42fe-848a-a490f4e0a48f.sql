...
DROP POLICY IF EXISTS "Anyone can update sessions" ON public.agent_sessions;
DROP POLICY IF EXISTS "Anyone can view sessions" ON public.agent_sessions;
...
CREATE POLICY "Users can insert their own sessions" ON public.agent_sessions FOR INSERT WITH CHECK (auth.uid() = user_id);
